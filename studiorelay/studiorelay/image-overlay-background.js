(() => {
    ...
    // Auto-fix missing extension for video downloads (e.g. ID -> ID.mp4)
    if (chrome.downloads && chrome.downloads.onDeterminingFilename) {
        chrome.downloads.onDeterminingFilename.addListener((item, suggest) => {
            try {
                let filename = item.filename || '';
                const isVideo = (item.mime && item.mime.includes('video')) ||
                                (item.url && (item.url.includes('tiktokcdn') || item.url.includes('dola') || item.url.includes('video')));
                if (isVideo && filename && !filename.toLowerCase().endsWith('.mp4')) {
                    suggest({ filename: filename + '.mp4', overwrite: false });
                    return;
                }
            } catch (e) {
                // ignore
