    ...
    <div className="panel flex min-h-[280px] flex-col p-4">
      ...
      {batchOpen && (
        <div className="mb-3 rounded-lg border border-white/10 bg-ink-900 p-3">
          <p className="mb-2 text-xs text-slate-400">Mỗi dòng là một prompt / một task.</p>
          <textarea
      ...
      {active ? (
          ...
          <div className="mt-2">
            ...
            <div className="flex flex-wrap gap-2">
              {active.refs.length === 0 && (
                <p className="text-xs text-slate-500">Chưa có ảnh tham chiếu.</p>
              )}
              {active.refs.map((ref) => (
                <div
                  key={ref.id}
                  className="grid h-16 w-16 place-items-center overflow-hidden rounded-lg border border-white/10 bg-ink-800 text-[10px] text-slate-400"
                >
                  {ref.url ? (
        ...
        <p className="text-sm text-slate-500">Chưa có task. Bấm Thêm Task để bắt đầu.</p>
      )}
    </div>
  );
}
