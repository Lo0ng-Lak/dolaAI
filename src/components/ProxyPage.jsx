  ...
  return (
    <div className={embedded ? "p-5" : "mx-auto h-full max-w-4xl overflow-auto"}>
      <div className={embedded ? "" : "panel p-5"}>
        ...
        <p className="mb-3 text-xs text-ink-700">{proxies.length} proxy · gắn vào từng tài khoản ở Quản lý tài khoản</p>

        <div className="space-y-2">
          {proxies.length === 0 && <p className="text-sm text-ink-700">Chưa có proxy.</p>}
          {proxies.map((p) => (
            <div
              key={p.id}
              className="flex items-center justify-between rounded-lg border border-line bg-slate-50 px-3 py-2"
            >
              <span className="min-w-0">
                <span className="block font-mono text-sm">{p.host}</span>
                <span className="text-[11px] text-ink-700">{p.status === "idle" ? "Sẵn sàng" : p.status || "Sẵn sàng"}</span>
              </span>
